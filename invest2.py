...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "points_display":
        ...
        admin_keyboard = [
            [
                InlineKeyboardButton("✅ Approve", callback_data=f"admin_approve_{invoice_number}"),
                InlineKeyboardButton("❌ Reject", callback_data=f"admin_reject_{invoice_number}")
            ]
        ]
        ...
        await query.edit_message_text(f"✅ Deposit #{invoice_number} approved!")
        ...
        try:
            user_message = f"❌ Deposit #{invoice_number} rejected. Contact admin."
            await context.bot.send_message(user_id, text=user_message)
        except Exception as e:
            logger.error(f"User notification failed: {e}")
        
        await query.edit_message_text(f"❌ Deposit #{invoice_number} rejected.")
...
async def handle_deposit_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Send to admin
        admin_keyboard = [[InlineKeyboardButton("📤 Send URL", callback_data=f"send_url_{invoice_number}")]]
        admin_reply_markup = InlineKeyboardMarkup(admin_keyboard)
        
        admin_message = (
            f"💰 NEW INVOICE #{invoice_number}\n\n"
            f"👤 User: {user_id}\n"
            f"💵 Amount: ${amount:.2f}\n"
        ...
        await creating_msg.edit_text(
            ...
            f"Amount: ${amount:.2f}\n"
            f"⏳ Admin will send payment URL shortly..."
        )
...
async def handle_admin_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_keyboard = [
        ...
        [InlineKeyboardButton("✅ Payment Done", callback_data=f"confirm_payment_{invoice_number}")]
    ]
    user_reply_markup = InlineKeyboardMarkup(user_keyboard)
    
    user_message = (
        f"💰 Deposit Invoice #{invoice_number}\n\n"
        f"💵 Amount: ${invoice['amount']:.2f}\n"
    ...
    try:
        ...
        await update.message.reply_text("❌ Failed to send URL to user.")
        return
    
    await update.message.reply_text(f"✅ URL sent to user #{user_id}")
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        user_message = f"✅ Withdrawal #{request_number} approved! New balance: {new_points:.2f} PTS"
