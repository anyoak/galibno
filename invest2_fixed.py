...
def verify_usdt_transaction(txid):
    """
    Verify USDT (BEP20) deposit by checking tokentx using txhash.
    - Uses BscScan 'tokentx' by txhash to find token transfer.
    - Ensures token contract matches USDT contract and 'to' matches DEPOSIT_ADDRESS.
    - Correctly handles USDT decimals (6).
    - Respects TIME_WINDOW_MINUTES (if set) but allows a generous default.
    Returns: (success: bool, amount: float, message: str)
    """
    try:
        api_url = "https://api.bscscan.com/api"

        # Query token transfers by txhash
        params = {
            'module': 'account',
            'action': 'tokentx',
            'txhash': txid,
        ...
        # Basic API response check
        if not data or data.get('status') != '1' or not data.get('result'):
            return False, 0, "Transaction not found or not confirmed on BscScan"

        # tokentx with txhash returns a list; pick the first matching transfer
        transfers = data.get('result', [])
        transfer = None
        for t in transfers:
            # match hash and contract
            if t.get('hash', '').lower() == txid.lower():
                transfer = t
                break

        if not transfer:
            return False, 0, "Token transfer record not found for this TXID"

        # Ensure this is USDT (BEP20)
        if transfer.get('contractAddress', '').lower() != USDT_CONTRACT_ADDRESS.lower():
            return False, 0, "Token contract does not match USDT (BEP20)"

        # Ensure tokens were sent to our deposit address
        if transfer.get('to', '').lower() != DEPOSIT_ADDRESS.lower():
            return False, 0, "USDT was not sent to the configured deposit address"

        # Convert amount: USDT (BEP20) uses 6 decimals
        try:
            value_int = int(transfer.get('value', '0'))
        except Exception:
            return False, 0, "Invalid transfer value"
        ...
        # Optional: check time window if TIME_WINDOW_MINUTES is set and > 0
        try:
            ...
            if TIME_WINDOW_MINUTES and TIME_WINDOW_MINUTES > 0:
                ...
                if current_time - tx_timestamp > TIME_WINDOW_MINUTES * 60:
                    ...
                    return False, 0, f"Transaction is {minutes_old} minutes old (max {TIME_WINDOW_MINUTES} minutes)"
        except Exception:
            # If timestamp parsing fails, don't block verification — just log
