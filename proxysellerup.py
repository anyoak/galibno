...
async def rotation_receive_package(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ok:
        await update.message.reply_text(
            proxy_mgr.style.error_message("FETCH ERROR", data),
            parse_mode='Markdown'
        )
    ...
    if not items:
        await update.message.reply_text(
            proxy_mgr.style.error_message("NO LISTS", "No IP lists found for this sub user."),
            parse_mode='Markdown'
        )
        context.user_data.pop("package_key", None)
        return ConversationHandler.END
...
async def perform_rotation_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not package_key or not list_id:
        await update.message.reply_text(
            proxy_mgr.style.error_message("ERROR", "Package/list not found. Please start again."),
            parse_mode='Markdown'
        )
...
async def handle_country_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("country_"):
        ...
        await context.bot.send_message(chat_id=query.message.chat_id, text=result, parse_mode='Markdown')
